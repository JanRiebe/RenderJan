...
Camera::Camera(Point pos, Point rot, float fov, int width, int height): position(pos), rotation(rot), fieldOfView(fov), imageWidth(width), imageHeight(height)
{
...
Ray Camera::PixelToRay(int x, int y)
{
  ...
  rayDirection = VecMatMult(rayDirection, cameraToWorld);
